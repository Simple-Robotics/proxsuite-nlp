...
void npy_save(std::string fname, const T *data, const std::vector<size_t> shape,
  ...
  size_t nels = std::accumulate(shape.begin(), shape.end(), 1UL,
...
void npz_save(std::string zipname, std::string fname, const T *data,
  ...
  uint32_t crc =
      (uint32_t)crc32(0L, (uint8_t *)&npy_header[0], (uInt)npy_header.size());
...
std::vector<char> create_npy_header(const std::vector<size_t> &shape) {
  ...
  std::size_t remainder = 16 - (10 + dict.size()) % 16;
