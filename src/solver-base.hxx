...
namespace proxnlp
{
  ...
  ) const
  {
    ...
    workspace.lamsPlusPre_data = workspace.lamsPrev_data + mu_inv_ * workspace.cstr_values_data;
  ...
  template<typename Scalar>
  void SolverTpl<Scalar>::computeResidualDerivatives(
    const ConstVectorRef& x,
    Workspace& workspace,
  ...
  template<typename Scalar>
  void SolverTpl<Scalar>::updatePenalty()
  {
    if (mu_ == mu_lower_)
    {
      setPenalty(mu_init_);
  ...
  void SolverTpl<Scalar>::solveInner(Workspace& workspace, Results& results)
  {
    ...
    auto phi_func = [&](const Scalar alpha) {
      ...
      return merit_fun(workspace.xTrial, workspace.lamsTrial, workspace.lamsPrev) + prox_penalty.call(workspace.xTrial);
    };
    ...
    for (k = 0; k < MAX_ITERS; k++)
    {
      ...
      workspace.kktMatrix.bottomRightCorner(ndual, ndual).diagonal().setConstant(-mu_);
      ...
      {
      case ARMIJO: {
        ArmijoLinesearch<Scalar>::run(phi_func, results.merit, workspace.dmerit_dir, verbose, ls_beta, armijo_c1, alpha_min, alpha_opt);
        break;
      }
      case CUBIC_INTERP: {
        CubicInterpLinesearch<Scalar>::run(phi_func, results.merit, workspace.dmerit_dir, verbose, armijo_c1, alpha_min, alpha_opt);
        break;
      }
      default: break;
      }
  ...
  template<typename Scalar>
  void SolverTpl<Scalar>::updateToleranceFailure()
  {
  ...
  template<typename Scalar>
  void SolverTpl<Scalar>::updateToleranceSuccess()
  {
