...
CostSum<Scalar> operator+(const CostFunctionBaseTpl<Scalar> &left,
                          const CostFunctionBaseTpl<Scalar> &right) {
...
// adding right
template <typename Scalar>
CostSum<Scalar> &&operator+(CostSum<Scalar> &&left,
                            const CostFunctionBaseTpl<Scalar> &right) {
  left += right;
...
template <typename Scalar>
CostSum<Scalar> operator*(const Scalar &left,
                          const CostFunctionBaseTpl<Scalar> &right) {
...
template <typename Scalar>
inline CostSum<Scalar> &&operator*(CostSum<Scalar> &&left, Scalar &&right) {
  return right * left;
}
} // namespace proxnlp
