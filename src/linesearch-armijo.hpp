...
template <typename _Scalar> struct ArmijoLinesearch {
  ...
  static void run(Fn phi, const Scalar phi0, const Scalar dphi0,
                  const VerboseLevel, const Scalar ls_beta,
                  const Scalar armijo_c1, const Scalar alpha_min,
                  Scalar &alpha_try) {
    alpha_try = 1.;
    ...
    do {
      ...
      if (std::abs(dphi0) < dphi_tresh) {
        return;
      }

      if (dM <= armijo_c1 * alpha_try * dphi0) {
        break;
      }
