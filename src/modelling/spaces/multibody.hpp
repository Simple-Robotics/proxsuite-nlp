...
public:
  ...
  MultibodyConfiguration(const ModelType &model) : m_model(model){};

  const ModelType &getModel() { return m_model; }

  PointType neutral() const { return pinocchio::neutral(m_model); }

  PointType rand() const { return pinocchio::randomConfiguration(m_model); }
