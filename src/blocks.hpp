...
struct SymbolicBlockMatrix {
	...
	void deep_copy(
			SymbolicBlockMatrix in, isize const* perm = nullptr) const noexcept {
		auto self = *this;
	...
	void brute_force_best_permutation(
			SymbolicBlockMatrix in, isize* best_perm, isize* iwork) const {
	...
	void llt_in_place() const noexcept {
      ...
      std::terminate();
		...
		// update l11
		for (isize i = 1; i < n; ++i) {
			self(i, i) = block_chol::add(
					self(i, i),
					block_chol::mul( //
							self(i, 0),
							block_chol::trans(self(i, 0))));

			for (isize j = i + 1; j < n; ++j) {

				self(i, j) = block_chol::add(
						self(i, j),
						block_chol::mul( //
								self(i, 0),
								block_chol::trans(self(j, 0))));
...
namespace backend {
template <typename M>
auto ref(M& mat) noexcept -> MatrixRef {
...
auto ref_submatrix(
		MatrixRef a, isize i, isize j, isize nrows, isize ncols) noexcept
		-> MatrixRef {
	return MatrixRef{
...
void ldlt_in_place(MatrixRef const& a) {
	ldlt_in_place_recursive(a);
}

template <BlockKind LHS, BlockKind RHS>
struct GemmT;

template <BlockKind LHS>
struct GemmT<LHS, Zero> {
	static void
	fn(MatrixRef const& /*dst*/,
	   MatrixRef const& /*lhs*/,
	   MatrixRef const& /*rhs*/,
	   Scalar /*alpha*/) {}
};
template <BlockKind RHS>
struct GemmT<Zero, RHS> {
	static void
	fn(MatrixRef const& /*dst*/,
	   MatrixRef const& /*lhs*/,
	   MatrixRef const& /*rhs*/,
...
template <>
struct GemmT<TriU, Dense> {
	// dst is dense
	static void fn(MatrixRef dst, MatrixRef lhs, MatrixRef rhs, Scalar alpha) {
		dst.noalias() +=
				(lhs.template triangularView<Eigen::Upper>() *
		     (alpha * rhs.transpose()));
	}
};

template <>
struct GemmT<Dense, Diag> {
	static void fn(MatrixRef dst, MatrixRef lhs, MatrixRef rhs, Scalar alpha) {
	...
	void ldlt_in_place_impl() const {
      ...
      std::terminate();
		...
		case TriL: {
          ...
          std::terminate();
				case TriU: {
					...
					// auto li0_tl = li0.transpose().template triangularView<Eigen::Lower>();
		...
		for (isize i = 1; i < nblocks; ++i) {
			...
			backend::gemmt(
					target_ii, li0, li0_prev, structure(i, 0), structure(i, 0), Scalar(-1));
			...
			for (isize j = i + 1; j < nblocks; ++j) {
				...
				backend::gemmt(
						target_ji, lj0, li0_prev, structure(j, 0), structure(i, 0), Scalar(-1));
