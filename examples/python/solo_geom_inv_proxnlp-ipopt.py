...
def vizGround(viz,elevation,space,name='ground',color=[1., 1., .6, 1.]):
...
if viz is not None: vizGround(viz, ground, .02)
...
def friction_cone_constraint(q, fs):
    ...
    for idx,force in zip(feet.keys(),fs):
        ...
        fc_constr.append(fp.T@fp - casadi.power(mu, 2) * (perp.T@force@force.T@perp@perp.T@perp))
...
def equality_constraints(q, fs):
    ...
    eq.append( torque )
    eq.append(com(q)[:2] - np.array([.1,.2]))
...
opti.subject_to( inequality_constraints(q, 'ipopt') <= 0)  # Sum of torques around COM is 0
...
fc_fun = CasadiFunction(pb_space.nx, pb_space.ndx, friction_cone_constraint(q, fs) , XF_s, use_hessian=False)
eq_fun = CasadiFunction(pb_space.nx, pb_space.ndx, equality_constraints(q, fs), XF_s, use_hessian=False)
ineq_fun = CasadiFunction(pb_space.nx, pb_space.ndx, inequality_constraints(q, 'proxnlp'), XF_s, use_hessian=False)
